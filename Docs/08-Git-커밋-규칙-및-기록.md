...
### 2026-08-03 — '이해' 키워드 포함 시 모든 귀신 만족 판정 추가

- 커밋: `16918b9` — `feat: 핵심 대답에 '이해' 포함 시 모든 귀신 만족 판정 추가`
- 바뀐 점: 핵심 고민 대답 시 답변에 '이해'가 포함되어 있으면 모든 귀신과 상담 회차에서 만족(해결 / 특별 해결) 반응을 받도록 로직을 추가했다.
- 주요 파일: `Assets/GhostCounselor/Scripts/GhostGameController.cs` — `Evaluate(answer, intent)` 함수 수정.
- 확인: `dotnet build Ghost.slnx --no-restore` 빌드 성공 (경고 0개, 오류 0개).
